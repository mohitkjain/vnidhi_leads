...
$app->get('/api/admin/target/previous_month/{user_id}', function ($request, $response) 
{
    ...
    if(isset($user_id))
    {
        try
        {
            ...
            $sql = "SELECT `target_amount` FROM `vn_target_fd` WHERE user_id = :user_id AND target_month = :pre_month AND target_year = :pre_year";
            ...
            if($stmt->execute())
            {
                ...
                $sql = "SELECT achieved  FROM vn_target_fd_achieved WHERE user_id = :user_id AND target_month = :pre_month AND target_year = :pre_year";
