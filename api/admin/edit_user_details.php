...
$app->post('/api/admin/users/user_edit', function ($request, $response) 
{
    ...
    if(isset($fname) && isset($lname) && isset($login) && isset($pass) && isset($usertype) && isset($position) && isset($empid) && isset($tl_id)  && isset($user_id))
    {
        try
        {
            $con = connect_db();
            ...
            $sql_update = "UPDATE `vn_users` SET `fname`= :fname,`lname`= :lname,`loginid`= :loginid,`password`= :password,`usertype`= :usertype,`empid`= :empid,`position`= :position,`tl_id`= :tl_id WHERE `user_id` =  :user_id";
